...
benchmark_gpu.py
================
Benchmark — GPU (CUDA) only — yolo26x
Unitree G1 EDU (Jetson Orin NX / JetPack 5.x)


Uso:
   python /home/unitree/Documents/Camera/benchmark_gpu.py
"""
...
MODELO     = "yolo26x.pt"
DEVICE     = "cuda"
N_AMOSTRAS = 6000          # ← número de frames a medir
N_WARMUP   = 5
...
def imprimir_relatorio(stats, mem_mb):
   print(f"\n{LINHA2}")
   print(f"  RESULTADO — {MODELO} @ {DEVICE.upper()}")
   print(LINHA2)
...
def main():
   if not torch.cuda.is_available():
       print("❌ CUDA não disponível. Use o script benchmark_cpu.py.")
   ...
   print(LINHA2)
   print(f"  BENCHMARK GPU — {MODELO}")
   print(f"  N_AMOSTRAS={N_AMOSTRAS}  |  N_WARMUP={N_WARMUP}")
   print(f"  Plataforma : {platform.platform()}")
   print(f"  Python     : {platform.python_version()}")
   print(f"  PyTorch    : {torch.__version__}")
   print(f"  GPU        : {torch.cuda.get_device_name(0)}")
   mem_total = round(torch.cuda.get_device_properties(0).total_memory / 1e6, 1)
   print(f"  Mem total  : {mem_total} MB")
   print(LINHA2)
   ...
   print(f"\n  📦 Carregando {MODELO} → CUDA...", end=" ", flush=True)
   model = YOLO(MODELO)
   model.to(DEVICE)
   print("OK")
   ...
   try:
       ...
       print(f"\n  ⏱️  Medindo {N_AMOSTRAS} frames: ", end="", flush=True)
       ...
       mem_mb = round(torch.cuda.memory_allocated() / 1e6, 1)
       torch.cuda.empty_cache()


       stats = calcular_estatisticas(tempos)
       stats["timestamp"]  = datetime.now().isoformat()
       stats["mem_gpu_mb"] = mem_mb


       imprimir_relatorio(stats, mem_mb)
       ...
       if ultimo_img is not None:
           ...
           cv2.putText(img_out, f"GPU | FPS: {stats['fps_media']}", (10,25),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,255), 2)
           cv2.putText(img_out, f"Inf: {stats['media_ms']:.0f}ms | {N_AMOSTRAS} frames", (10,50),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,255), 2)
           cv2.putText(img_out, f"GPU Mem: {mem_mb} MB", (10,75),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,255), 2)


           fname = Path(__file__).parent / "benchmark_gpu_resultado.jpg"
           cv2.imwrite(str(fname), img_out)
           print(f"  💾 Imagem salva: {fname}")


       # Salva JSON
       with open(f"{OUTPUT_DIR}/benchmark_gpu_resultado.json", "w") as f:
           json.dump({
               "modelo"    : MODELO,
               "device"    : DEVICE,
       ...
       print(f"\n  ✅ Resultados salvos em {OUTPUT_DIR}/benchmark_gpu_resultado.json")
